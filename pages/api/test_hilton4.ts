...
const areas = ["Niseko"];
// const areas = ["Niseko","Tokyo","Toyama","Nagoya","Osaka","Hiroshima","Fukuoka","Nagasaki","Okinawa","Miyakojima"];
    ...
    for (let dateNum = 0; dateNum < capture_date_count; dateNum++) {
        ...
        try{
            for (let areaNum = 0; areaNum < areas.length; areaNum++) {
                ...
                // 検索URL決定  
                const searchUrl = `https://www.hilton.com/en/search/?query=${area},%20${country_code}&arrivalDate=${cid}&departureDate=${cod}`
        
                console.log(`Start capturing ${group_code} : ${cid} : ${area} area rates...`);
                ...
                // await page.waitForSelector("li[data-testid*='hotel-card-']", { timeout: 30000 });
                await new Promise(resolve => setTimeout(resolve, 5000));
                const test = await page.evaluate(()=>{
                    return document.querySelector("p[data-testid='priceInfo']");
                })
                console.log(test);

                // const rate_list: Rate[] 
                const hotel_node_list = await page.evaluate((cid: string, capture_date: string, hotels:{id:string,hotel_code:string}[])=>{
                    const rateByHotels:Rate[] = [];
                    const hotel_node_list = document.querySelector("p[data-testid='priceInfo']");
                    // const hotel_node_list = [1,2,3,4,5];
                    ...
                    //     }
                    // }
                    return hotel_node_list
                },cid,capture_date,hotels);

                console.log(hotel_node_list);
