...
import { fetchHotelCodeById } from '@/app/lib/data'
...
const captureRates = async (puppeteer: any, chrome:any={}, hotel_id: string, cim: string) => {

    // const hotel_id = "44b036c0-6f4a-4912-9574-10c279df9cca"; // reqからとってくる？
    // const cim = "2023-11";
    ...
    // 取得対象ホテル設定
    const hotel_code = await fetchHotelCodeById(hotel_id)
    ...
    const capture_month = new Date(cim).toLocaleDateString("ja-JP", {year: "numeric",month: "2-digit"}).replace('/', '-');
...
export default async function handler(
  ...
  res: NextApiResponse<string>
    ...
    try {
        ...
        const { hotel_id, cim} = req.query
        await captureRates(puppeteer, chrome, hotel_id as string, cim as string);
